		...
		<div class="bg-white rounded-lg shadow dark:border dark:bg-gray-800 dark:border-gray-700">
			<div class="p-6 space-y-4 md:space-y-6 sm:p-8">
				...
				<form class="space-y-4 md:space-y-12" action="<?= base_url(); ?>client/editar/<?= $data['id_cliente'] ?>" method="post">
					...
					<div>
						<label for="countries" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Membresia</label>
						<select name="id_membresia" id="countries" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
							...
							<?php
							if (count($dataSend) > 0) :
								foreach ($dataSend as $row) :
									$selected = '';
									if ($data['id_membresia'] == $row['id_membresia']) {
										$selected = 'selected';
									}

							?>

									<option <?= $selected ?> value="<?= $row['id_membresia'] ?>"><?= $row['descripcion'] ?> Precio: <?= $row['precio'] ?></option>
							<?php
								endforeach;
							endif;
							?>
