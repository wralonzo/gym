	...
	<div class="justify-center">
		<div class="bg-white rounded-lg shadow dark:border dark:bg-gray-800 dark:border-gray-700">
			<div class="p-6 space-y-4 md:space-y-6 sm:p-8">
				...
				<form class="space-y-4" action="<?=  base_url(); ?>horario/editar/<?= $data['id_horario'] ?>" method="post">
				<div>
					...
					<div>
						...
						<input value="<?=$data['hora_inicio'] ?>" type="time" name="hora_inicio" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Descripcion" required="">
					</div>

					<div>
						<label for="apellidos" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Hora final</label>
						<input value="<?=$data['hora_fin'] ?>" type="time" name="hora_fin" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Descripcion" required="">
					</div>
						...
						<select name="id_user" id="countries" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
							...
							if (count($usuarios) > 0) :
								foreach ($usuarios as $row) :
									$selected = '';
									if($data['id_user'] == $row['id'] ){
										$selected = 'selected';
									}
							?>
									<option <?= $selected ?> value="<?= $row['id'] ?>"><?= $row['nombre'] ?> <?= $row['apellidos'] ?></option>
							<?php
								endforeach;
							endif;
							?>
					...
					<div class="flex items-center mb-4">
						...
						if (count($dias) > 0) :
							...
							foreach ($dias as $dia) :
								if ($count % 2 == 0) :
									$checked = getChecked($data['id_horario'],$dia['id_dia'] );
						?>
									<input <?= $checked ?> id="<?= $dia['id_dia'] ?>" name="dias[]" type="checkbox" value="<?= $dia['id_dia'] ?>" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
